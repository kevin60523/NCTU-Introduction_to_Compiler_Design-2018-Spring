...
void insertTab( struct SymTable *table, struct SymNode *newNode,int num )
{
	int location = 0;
...
struct SymNode* createParamNode( const char *name, int scope, struct PType *type ,int num)
{
	struct SymNode *newNode = (struct SymNode *)malloc( sizeof(struct SymNode) );
	/* setup name */
	newNode->name = (char *)malloc(sizeof(char)*(strlen(name)+1));
